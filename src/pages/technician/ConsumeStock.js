...
function ManageStocks() {
  ...
  const handleUpdate = async () => {
    ...
    const UpdatedData = {
      ...
      consumed_stock: parseInt(editedData.current_stocks),
    };
    console.log(UpdatedData);
    try {
      const response = await axios.put(
        "http://localhost:8080/protected/consume-stock",
        UpdatedData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      ...
      <Modal isOpen={isEditModalOpen} onClose={closeEditModal}>
        <ModalHeader>Consume Stock</ModalHeader>
        <ModalBody>
          <Label className="mb-2">No of Quantity</Label>
          <Input
            name="current_stocks"
            placeholder="10"
            value={editedData.current_stocks || ""}
            onChange={(e) =>
              setEditedData({ ...editedData, current_stocks: e.target.value })
            }
            className="mb-4"
          />
        </ModalBody>
